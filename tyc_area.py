...
class Tianyancha():
    ...
    def __init__(self, username, password):
        ...
        # proxy = "--proxy-server=http://" + '112.113.152.48:4256'
    ...
    def get_city(self,shen_link):
        ...
        for city in city_list:
            ...
            self.get_qu(city_link)
        return self.driver

    def get_qu(self,city_link):
        self.driver.get(city_link)
        doc = etree.HTML(self.driver.page_source.encode('utf-8'))
        qu = doc.xpath("//div[@id='prov_box']/div[3]/div[@class='content']/div[@class='item']")
        if len(qu) == 1:
            qu_list = doc.xpath("//div[@id='prov_box']/div[3]/div[@class='content']/div[@class='item']")
        else:
            qu_list = doc.xpath("//*[@id='prov_box']/div[3]/div[2]/div[@class='item' and position()>1]")
        for qu in qu_list:
            qu_link = qu.xpath("./a/@href")[0]
            self.parse(qu_link)
        return self.driver
    ...
    def parse(self,link):
        ...
        for div in divs:
            ...
            item["Additional"] = ''.join(Additional) if len(Additional) > 0 else None
    ...
    def save_to_mongo(self,data):
        if self.db['孵化器'].update({'企业名称': data['企业名称']}, {'$set': data}, True):
